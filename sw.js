...
const SHELL=['./','./index.html','./manifest.webmanifest','./icon-192.png','./icon-512.png','./lifeos-patch.js'];
...
async function withPatch(r){
  try{
    const txt=await r.text();
    if(txt.indexOf('lifeos-patch.js')>=0)return new Response(txt,{headers:r.headers,status:r.status,statusText:r.statusText});
    const patched=txt.replace('</body>','<script src="lifeos-patch.js?v=5"></script></body>');
    return new Response(patched,{headers:{'Content-Type':'text/html;charset=utf-8'},status:r.status,statusText:r.statusText});
  }catch(_){return r;}
}
self.addEventListener('fetch',function(e){
  ...
  if(isHtml){
    e.respondWith((async function(){
      const c=await caches.open(CACHE);
      try{const r=await fetch(req,{cache:'no-store'});if(r&&r.ok){const p=await withPatch(r.clone());c.put(req,p.clone());return p;}}catch(_){}
      const hit=await c.match(req,{ignoreVary:true});return hit?withPatch(hit):Response.error();
    })());
    return;
  }
