...
async function withPatch(r){try{const txt=await r.text();if(txt.indexOf('lifeos-patch.js')>=0)return new Response(txt,{headers:r.headers,status:r.status,statusText:r.statusText});const patched=txt.replace('</body>','<script src="lifeos-patch.js?v=25"></script></body>');return new Response(patched,{headers:{'Content-Type':'text/html;charset=utf-8'},status:r.status,statusText:r.statusText});}catch(_){return r;}}
